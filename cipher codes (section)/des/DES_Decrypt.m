function Plain_txt=DES_Decrypt(Cipher_txt,Key)
%%  2. key schedule
% define permuted choice 1 (PC1)
...
for J=1:16
    C(J+1,:)=circshift(C(J,:),-Left_Shift(J),2);
    D(J+1,:)=circshift(D(J,:),-Left_Shift(J),2);
    Key_LCS=[C(J+1,:) D(J+1,:)];      
    for I=1:length(PC2)
...
for I=1:length(IP)
    Cipher_txt_IP(I)=Cipher_txt(IP(I));
end
%% Step2: Divide Plain_txt_IP into two halves Left and Right
L(1,:)=Cipher_txt_IP(1:32);       
R(1,:)=Cipher_txt_IP(32+1:64);
...
for J=1:16
    ...
    %Step2: 
    R_Expn=xor(R_Expn,Key_Round(17-J,:));%48 bit
    %Step3: Perform Substitution/Choice S-Box
    for I=1:8        
        S_Input=R_Expn(6*I-5:6*I);
        S_Row_Bin=[S_Input(1) S_Input(6)];
        ...
        elseif I==7
            ...
            S=S8(S_Row_Dec+1,S_Col_Dec+1);
        end
        S_Out(1,4*I-3:4*I)=dec2bin(S,4);
    end
